...
def generate_random_valid_username(website, length):
    ...
    for random_username in generate_random_username(website, length):
        if username_checker.check_format(website, random_username):
...
def get_response(app, website, user):
    resp_bytes = app.get('/api/check/{}/{}'.format(website, user))
...
def get_expected_response(website, user, status):
    return {
        ...
        'usable': True,
        'status': status,
        'url': username_checker.get_profile_url(website, user),
        'avatar': username_checker.get_avatar(website, user) if status == 200
        else None
    }
...
class TestUsernameApi(object):
    ...
    def setup_class(self):
        self.app = username_checker.app.test_client()
    ...
    def test_format_checking(self, website, username):
        resp = self.app.get('/api/check/{w}/{u}'.format(w=website, u=username))
        json_resp = json.loads(resp.get_data().decode())
        url = username_checker.get_profile_url(website, username)
        assert {
            'possible': False,
            'usable': True,
            'url': url
